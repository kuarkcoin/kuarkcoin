...
export async function getKapImportant(options: { mode?: Mode } = {}) {
  ...
  try {
    ...
    const r = await fetchWithTimeout("https://www.kap.org.tr/tr/api/memberDisclosureQuery", {
      method: "POST",
      headers: {
    ...
    return { ok: false, error: msg, items: [], meta: { reason: e?.name === "AbortError" ? "TIMEOUT" : "FETCH_ERROR" } };
  }
