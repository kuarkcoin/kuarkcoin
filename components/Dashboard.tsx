...
export default function Dashboard({ signals, topBuy, topSell }: any) {
  ...
  return (
    <div className="p-6 space-y-6 bg-[#0d1117] min-h-screen text-white">
      <div className="grid grid-cols-1 md:grid-cols-3 gap-4">
        <div className="bg-[#161b22] p-5 rounded-2xl border border-gray-800 shadow-xl">
          ...
          <div className="w-full bg-gray-800 h-1.5 rounded-full mt-4 overflow-hidden">
            <div className="bg-blue-500 h-full transition-all duration-1000" style={{ width: `${marketSentiment}%` }} />
          </div>
        </div>

        <div className="bg-[#161b22] p-5 rounded-2xl border border-gray-800">
          <div className="text-xs text-gray-500 uppercase font-bold tracking-widest">En Güçlü Alım</div>
          <div className="mt-2 text-xl font-bold text-green-400">
            {topBuy?.[0]?.symbol ? symbolToPlain(topBuy[0].symbol) : "—"}
          </div>
          <div className="text-xs text-gray-500 mt-1">Skor: {topBuy?.[0]?.score ?? 0}</div>
        </div>
        ...
        <div className="bg-[#161b22] p-5 rounded-2xl border border-gray-800">
          ...
          <div className="text-xs text-gray-500 mt-1">Skor: {topSell?.[0]?.score ?? 0}</div>
        </div>
      </div>
